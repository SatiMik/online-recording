...
export default function OnlineMasterServiceCard({ service, master }: MasterServiceCardProps): JSX.Element {
        ...
        <Box sx={{ display: 'flex' }}>
            ...
            <Card sx={{ width: 400, boxShadow: 10, marginBottom: '20px' }}>
                <CardContent sx={{ display: 'flex', }}>
                    ...
                    <RadioGroup sx={{ margin: '20px', display: 'flex', justifyContent: 'space-between' }}
                    ...
                    >
                        {dateArray?.map((date) => <FormControlLabel value={date} control={<Radio />} label={date} />)}
                    </RadioGroup>
                    <RadioGroup sx={{ margin: '20px', display: 'flex', justifyContent: 'space-between' }}
                        aria-labelledby="demo-controlled-radio-buttons-group"
                        name="controlled-radio-buttons-group"
                    ...
                    >
                        ...
                        {timeArray?.map((time) => <FormControlLabel key={time} value={time} control={<Radio />} label={time} />)}
                    </RadioGroup>
                    <Button variant="contained" sx={{ marginLeft: 'auto', height: '100%', alignSelf: 'center' }} onClick={() => void dispatch(addMasterServicesThunk(inputRecord))}>Записаться</Button>
                </CardContent>
            </Card>
        </Box >
    )
